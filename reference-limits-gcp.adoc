...
Nein, das tut es nicht.  Daten in einem HA-Paar werden synchron zwischen den Knoten gespiegelt, sodass die Daten im Falle eines Ausfalls in Google Cloud verfügbar sind.  Wenn Sie beispielsweise eine 8-TB-Festplatte auf Knoten A kaufen, weist BlueXP auch eine 8-TB-Festplatte auf Knoten B zu, die für gespiegelte Daten verwendet wird.  Obwohl eine Kapazität von 16 TB eingerichtet ist, werden nur 8 TB auf das Lizenzlimit angerechnet.
